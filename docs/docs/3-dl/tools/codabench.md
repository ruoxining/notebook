...
[Codabench](https://www.codabench.org/){target="_blank"} 是一个为数据科学和机器学习比赛设计的低代码模型测试平台。平台提供 CPU，允许比赛创办者通过上传脚本和数据的方式创办比赛，并允许其它用户使用平台 CPU 来运行代码得到结果。前序工作是 [CodaLab](https://codalab.org/){target="_blank"}，2023 年 Codabench 团队仿照 CodaLab 改进出了 Codabench。
...
[codalab/competition-examples/codabench](https://github.com/codalab/competition-examples/tree/master/codabench){target="_blank"} 官方提供的几个 bundle 模板，可以对比 Codabench 上同名比赛查看创建结果，在本仓库拿一些需要的写法。
...
- [Codabench: Flexible, Easy-to-Use and Reproducible Benchmarking Platform](https://arxiv.org/abs/2110.05802){target="_blank"}
- [Codabench Repo Wiki](https://github.com/codalab/codabench/wiki){target="_blank"}
- [Codabench Repo Issue](https://github.com/codalab/codabench/issues){target="_blank"}
